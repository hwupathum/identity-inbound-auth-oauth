...
public class UserInfoResponseBaseTest extends PowerMockTestCase {
    ...
    /**
     * Turn on the default-off gate knob {@value #RECOMPUTE_SUBJECT_CLAIM_KNOB} for a single test method.
     * When the knob is left untouched, {@code IdentityUtil.getProperty} returns null and the knob stays off,
     * preserving the pre-fix behaviour.
     */
    protected void enableRecomputeSubjectClaimKnob() {

        mockStatic(IdentityUtil.class);
        when(IdentityUtil.getProperty(RECOMPUTE_SUBJECT_CLAIM_KNOB)).thenReturn("true");
    }

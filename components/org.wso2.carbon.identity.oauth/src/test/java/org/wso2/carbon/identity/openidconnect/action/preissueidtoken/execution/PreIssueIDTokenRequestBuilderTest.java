...
public class PreIssueIDTokenRequestBuilderTest {
    ...
    public void setUp() throws OrganizationManagementException {
        ...
        org.wso2.carbon.identity.organization.management.service.model.Organization organizationModel =
                new org.wso2.carbon.identity.organization.management.service.model.Organization();
        organizationModel.setId(ORGANIZATION_ID_TEST);
        organizationModel.setName(ORGANIZATION_NAME_TEST);

        OrganizationManager organizationManager = mock(OrganizationManager.class);
        when(organizationManager.getOrganization(eq(ORGANIZATION_ID_TEST), eq(false), eq(false)))
                .thenReturn(organizationModel);
        when(organizationManager.getOrganization(eq(USER_RESIDENT_ORG_ID), eq(false), eq(false)))
                .thenReturn(organizationModel);
        when(organizationManager.getOrganization(eq(ACCESSING_ORG_ID), eq(false), eq(false)))
                .thenReturn(organizationModel);

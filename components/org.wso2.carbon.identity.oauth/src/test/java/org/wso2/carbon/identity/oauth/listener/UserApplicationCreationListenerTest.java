...
public class UserApplicationCreationListenerTest extends IdentityBaseTest {
    ...
    public void testDoPostAddUserWithID_ApplicationNameWithAgentNameFromClaims()
        ...
        boolean result = listener.doPostAddUserWithID(user, "password",
                new String[]{"role1"}, claims, null, userStoreManager);

        assertTrue(result,
                "Listener should return true after successful application creation");

        ArgumentCaptor<ApplicationDTO> applicationDTOCaptor =
                ArgumentCaptor.forClass(ApplicationDTO.class);
        verify(applicationManagementService).createApplication(
                applicationDTOCaptor.capture(), eq(TENANT_DOMAIN), eq(username));

        ApplicationDTO capturedAppDTO = applicationDTOCaptor.getValue();
        assertNotNull(capturedAppDTO, "ApplicationDTO should not be null");
        assertNotNull(capturedAppDTO.getServiceProvider(),
                "ServiceProvider should not be null");

        String applicationName =
                capturedAppDTO.getServiceProvider().getApplicationName();
    ...
    public void testDoPostAddUserWithID_ApplicationNameWithNullClaims()
        ...
        boolean result = listener.doPostAddUserWithID(user, "password",
                new String[]{"role1"}, null, null, userStoreManager);

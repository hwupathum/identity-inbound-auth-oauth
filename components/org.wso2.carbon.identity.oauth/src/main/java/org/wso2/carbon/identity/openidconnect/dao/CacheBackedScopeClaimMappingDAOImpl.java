...
public class CacheBackedScopeClaimMappingDAOImpl implements ScopeClaimMappingDAO {
    ...
    private OIDCScopeClaimCacheEntry loadOIDCScopeClaimsOnRead(int tenantId, OIDCScopeClaimCacheEntry
        ...
        if (oidcScopeClaimCacheEntry == null || oidcScopeClaimCacheEntry.getScopeClaimMapping().size() == 0) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("[AddToCacheOnRead][AddScopeClaimMapOnRead] OIDC scopes and mapped claims are loaded " +
                        "from the database and inserted to the cache for the tenant : " + tenantId);
            }
        }
        return oidcScopeClaimCacheEntry;
    }

...
/**
 * The ImpersonatorPermissionValidator class is responsible for validating whether an authenticated user has the necessary
 * impersonation permissions within a given tenant and for a specified client. This class implements the ImpersonationValidator
 * interface and uses a DefaultOAuth2ScopeValidator to validate the requested scopes.
 *
 * The validation process involves checking if the authenticated user has the "IMPERSONATION_SCOPE_NAME" in their authorized
 * scopes. If the scope is present, the impersonation context is marked as validated. Otherwise, the validation fails, and an
 * appropriate error message is logged and set in the impersonation context.
 *
 */

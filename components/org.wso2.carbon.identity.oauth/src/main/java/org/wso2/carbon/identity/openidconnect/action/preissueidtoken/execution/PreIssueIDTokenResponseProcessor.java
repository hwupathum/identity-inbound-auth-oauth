...
public class PreIssueIDTokenResponseProcessor implements ActionExecutionResponseProcessor {
    ...
    private boolean isServerError(String errorCode) {
        ...
        return ("internal_server_error".equalsIgnoreCase(errorCode) ||
                "server_error".equalsIgnoreCase(errorCode) ||
                String.valueOf(HttpStatus.SC_INTERNAL_SERVER_ERROR).equalsIgnoreCase(errorCode));
    }
    ...
    private OperationExecutionResult addAudience(PerformableOperation operation,
        ...
        AbstractToken.Claim audience = requestIDToken.getClaim(IDToken.ClaimNames.AUD.getName());
        ...
        //todo: In the add path it should be possible to add audience irrespective of the fact the id token
    ...
    private OperationExecutionResult replaceClaim(PerformableOperation operation, IDToken requestIDToken,
        ...
        if ((CLAIMS_PATH_PREFIX + IDToken.ClaimNames.EXPIRES_IN.getName()).equals(operation.getPath())) {
            return replaceExpiresIn(operation, responseIDTokenDTO);
        } else if (operation.getPath() != null &&
                operation.getPath().startsWith(CLAIMS_PATH_PREFIX + IDToken.ClaimNames.AUD.getName())) {
    ...
    private OperationExecutionResult replaceClaimValueAtIndexFromArrayTypeClaim(PerformableOperation operation,
        ...
        // Replace claim value in the response id token
